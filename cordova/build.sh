...
# Configs
cd node_modules/Exoduscore
babel ./*.js -d babel
cp -rf babel/* .
rm -rf babel
cd ../..
cd node_modules/Exoduscore/HDWallet
babel ./*.js -d babel
cp -rf babel/* .
rm -rf babel
cd ../../..
cd node_modules/Exoduscore/model
babel ./*.js -d babel
cp -rf babel/* .
rm -rf babel
cd ../../..
BUILDDIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
PROJECT="$BUILDDIR/../../Exodusbuilds/project-$1"
if [ ! -d "$BUILDDIR/../../Exodusbuilds" ]; then
    mkdir -p $BUILDDIR/../../Exodusbuilds
fi
...
if [ ! -d $PROJECT ]; then
	...
	cordova create ../../Exodusbuilds/project-$1 com.Exodus.wallet Exodus
	checkOK
...
#    checkOK
	...
	cordova plugin add cordova-plugin-customurlscheme --variable URL_SCHEME=Exodus
	checkOK
...
if $DBGJS; then
	echo "${OpenColor}${Green}* Generating Exodus bundle (debug js)...${CloseColor}"
	cd $BUILDDIR/..
	grunt cordova
	checkOK
else
	echo "${OpenColor}${Green}* Generating Exodus bundle...${CloseColor}"
	cd $BUILDDIR/..
	grunt cordova-prod
	checkOK
fi
...
cp node_modules/Exoduscore/initial.Exodus.sqlite $PROJECT/www
cp node_modules/Exoduscore/initial.Exodus-light.sqlite $PROJECT/www
checkOK
...
if [ $CURRENT_OS == "ANDROID" ]; then
	...
	cat $BUILDDIR/android/android.css >> $PROJECT/www/css/Exodus.css
...
if [ $CURRENT_OS == "IOS" ]; then
    ...
    cat $BUILDDIR/ios/ios.css >> $PROJECT/www/css/Exodus.css
...
#  checkOK
#
#  cp ios/Exodus-Info.plist $PROJECT/platforms/ios/Exodus-Info.plist
#  checkOK
#
#  mkdir -p $PROJECT/platforms/ios/Exodus/Resources/icons
#  checkOK
#
#  mkdir -p $PROJECT/platforms/ios/Exodus/Resources/splash
#  checkOK
#
#  cp -R ios/icons/* $PROJECT/platforms/ios/Exodus/Resources/icons
#  checkOK
#
#  cp -R ios/splash/* $PROJECT/platforms/ios/Exodus/Resources/splash
#  checkOK
fi
