...
angular.module('copayApp.services').factory('go', function($window, $rootScope, $location, $state, profileService, fileSystemService, nodeWebkit, notification, gettextCatalog, authService, $deepStateRedirect, $stickyState, configService, gettext) {
    ...
    var toggleSidebar = function(exort) {
        if (typeof document === 'undefined')
            return;
    ...
    function handleUriAddr(uri) {
        if(uri.indexOf("Exodus-EXO:") != -1){
            uri = uri.replace('Exodus-EXO:','');
            if(uri.length === 32 ) {
                $rootScope.$emit('Local/ShadowAddress',uri);
            }            //冷钱包二维码验证
        }else if(uri.indexOf("shadow") != -1){
            require('Exoduscore/shadowUri.js').shadowParseUri(uri,{
    ...
    function handleUri(uri){
        //if (uri.indexOf("Exodus:") == -1 ) return handleFile(uri);
        //if (uri.indexOf("Exodus:") == -1 ) return handleFile(uri);
        ...
        if(uri.indexOf("shadow") != -1){
            ...
            require('Exoduscore/shadowUri.js').isHotParseUri(uri, {
                ifError: function(err){
                    console.log(err);
        ...
        //else return handleFile(uri);
            ...
            require('Exoduscore/uri.js').parseUri(uri, {
                ifError: function(err){
                    console.log(err);
                    notification.error(err);
                    //notification.success(gettextCatalog.getString('Success'), err);
    ...
    function handleFile(uri) {
        ...
        if (uri.indexOf("content:") !== -1) {
            window.plugins.intent.readFileFromContentUrl(uri.replace(/#/g,'%23'), function (content) {
                ...
                //todo delete
                // require('Exoduscore/wallet.js').handlePrivatePaymentFile(null, content, cb);
        ...
        if (uri.indexOf("." + configService.privateTextcoinExt) != -1) {
            ...
            //todo delete
            // require('Exoduscore/wallet.js').handlePrivatePaymentFile(uri, null, cb);
            return checkDoubleClaim();
        }
        $rootScope.$emit('process_status_change', 'claiming', false);
    }

    function extractExodusArgFromCommandLine(commandLine){
...
X-Ubuntu-StageHint=SideStage\n", {mode: 0755}, function(err){
	 ...
	 <mime-info xmlns='http://www.freedesktop.org/standards/shared-mime-info'>\n\
	   ...
	   <comment>Exodus Private Coin</comment>\n\
    ...
    if (gui){ // nwjs
        var removeListenerForOnopen = $rootScope.$on('Local/BalanceUpdatedAndWalletUnlocked', function(){
            ...
            gui.App.on('open', function(commandLine) {
                ...
                if (commandLine){
                    var file = extractExodusArgFromCommandLine(commandLine);
                    if (!file)
                        return console.log("no Exodus: arg found");
                    handleUri(file);
