...
angular.module('copayApp.controllers').controller('walletnameController',
    function($rootScope, $scope, $timeout, profileService, go, gettext, gettextCatalog, $state, $stateParams, isCordova, notification, storageService, configService, nodeWebkit) {
        ...
        self.deleteWallet = function(walletId,name) {
            ...
            //console.log('fc=========',fc)
            /**
             * 删除钱包时，判断是否只有一个EXO钱包，如果只有一个，则不然删除EXO钱包
             * @type {number}
             */
            let sum = 0;
            fc.forEach(function (item) {
                if(item.type =='EXO')sum ++;
            })
            if (sum == 1 && walletId.split('-')[0] == 'EXO')
                return $rootScope.$emit('Local/ShowErrorAlert', gettextCatalog.getString("Can't delete the last remaining EXO wallet"));
            else self.showconfirm = true;
            // require('Exoduscore/wallet').findFirstAddress(function (res) {
            //     if(walletId == res.wallet)return $rootScope.$emit('Local/ShowErrorAlert', gettextCatalog.getString("Can't delete the Main EXO wallet"));
        ...
        self.truedeleteWallet = function(walletId,name,address) {
            ...
            if(!name) {
                ...
                var walletDefined = require('Exoduscore/wallet_defined_by_keys');
