...
angular.module('copayApp.controllers').controller('createwalletController',
    function ($rootScope, $scope, $timeout, storageService, notification, profileService, bwcService, $log,gettext,go,gettextCatalog,isCordova) {
        ...
        self.createWallet= function (walletName, password, passphrase, mnemonic,del) {
            ...
            // $scope.loading = true;
            setTimeout(function () {
                profileService.createNewWallets({ walletName: walletName, password: passphrase, mnemonic: mnemonic, type:'EXO' }, function (err) {
                    $timeout(function () {
                        if (isCordova)
        ...
        self.importw = function(){
            ...
            setTimeout(function () {
                profileService.createNewWallets({ walletName: self.createwiname, password: self.createwipass, mnemonic: self.importcode ,type:'EXO'}, function (err) {
                    $timeout(function () {
                        if (isCordova)
                            window.plugins.spinnerDialog.hide();
                        else
