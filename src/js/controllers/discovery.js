...
angular.module('copayApp.controllers').controller('discoveryController',
    function ($rootScope, $scope, $timeout, lodash, isCordova, $modal, storageService, profileService, gettext, gettextCatalog, nodeWebkit, $sce) {
        ...
        var payment = require('Exoduscore/payment.js');
        var webHelper = require('Exoduscore/webhelper.js');
        ...
        self.dappList = []
        self.address = '' // 当前地址
        self.address = $scope.index.walletType ? $scope.index.walletType.EXO[0].address : ''; // 获取第一个EXO地址
        ...
        // let walletId = $scope.index.walletType.EXO[0].wallet;
        let walletId = $scope.index.walletType ? $scope.index.walletType.EXO[0].wallet : '';
        ...
        self.currencyData = function (upyn) {
            ...
            news.getExoData2(function (res) {
                if (!!res && res != null) {
                    self.coinexolist = res.page.list;
                }
            });
        ...
        self.getexRate = function () {
            news.getExoData2(function (res) {
                if (!!res && res != null) {
                    //self.exodollar = res.page.list.EXO.price;
                    $scope.index.exodollar = res.page.list.EXO.price;
                    //self.exormb = res.page.list.EXO.cnyPrice;
                    $scope.index.exormb = res.page.list.EXO.cnyPrice;
                    $timeout(function () {
                        $scope.$apply();
                    })
                }
        ...
        const eventBus = require('Exoduscore/event_bus.js');
            ...
            profileService.setAndStoreFocusToWallet(walletId, function () {
                profileService.unlockFC(null, function (err) {
                    ...
                    payment.transactionMessage(objJson, function (err, res2) {
                        ...
                        // obj.localfullnode_list = [require('Exoduscore/conf').URL.EXO_TRANSACTION_URL];//IP+httpPort
        ...
        function _getDappList() {
            var dapp = require('Exoduscore/dapp');
