...
angular.module('copayApp.controllers').controller('exportController',
	function($rootScope, $scope, $timeout, $log, $filter, backupService, storageService, fileSystemService, isCordova, isMobile, gettextCatalog, notification) {
		...
		var conf = require('Exoduscore/conf');
		var zip;
		if (isCordova) {
		...
		function saveFile(file, cb) {
			var backupFilename = 'ExodusBackup-' + $filter('date')(Date.now(), 'yyyy-MM-dd-HH-mm-ss') + '.encrypted';
			if (!isCordova) {
				var inputFile = document.getElementById('nwExportInputFile');
				...
				fileSystemService.cordovaWriteFile((isMobile.iOS() ? window.cordova.file.cacheDirectory : window.cordova.file.externalRootDirectory), 'Exodus', backupFilename, file, function(err) {
					if (isMobile.iOS()) {
						navigator.notification.alert(gettextCatalog.getString('Now you have to send this file somewhere to restore from it later ("Save to Files", send to yourself using chat apps, etc.)'), function(){
							window.plugins.socialsharing.shareWithOptions({files: [window.cordova.file.cacheDirectory + 'Exodus/'+ backupFilename]}, function(){}, function(){});
						}, 'Backup done');
					}
					cb(err);
				});
