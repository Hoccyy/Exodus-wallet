...
var constants = require('Exoduscore/constants.js');
var eventBus = require('Exoduscore/event_bus.js');
var breadcrumbs = require('Exoduscore/breadcrumbs.js');
var ValidationUtils = require('Exoduscore/validation_utils.js');
...
angular.module('copayApp.controllers')
    .controller('walletHomeController', function($http, $scope, $rootScope, $timeout, $window, $state, $stateParams, $filter, $modal, $log, notification, isCordova, profileService, lodash, configService, storageService, gettext, gettextCatalog, nodeWebkit, addressService, confirmDialog, go) {
        ...
        //todo delete
        // var chatStorage = require('Exoduscore/chat_storage.js');
        ...
        this.shareAddress = function(addr) {
            if (isCordova) {
                ...
                window.plugins.socialsharing.shareWithOptions({message: "My Exodus address " + self.protocol +  ':' + addr, subject: "My Exodus address"/*, url: self.protocol +  ':' + addr*/}, function(){}, function(){});
            }
        };
        ...
        var btcrpc = require('Exoduscore/HDWallet/btc_rpcHelper');
        var ethrpc = require('Exoduscore/HDWallet/eth_rpchelper');
        var ethHelper = require('Exoduscore/HDWallet/eth_helper');
        /**
         * 获取交易
         * @type {{BTCTo: BTCTo}}
         */
        var getHash = {
            ...
            "EXOTo" : function (object, mnemonic, from_address, to_address,amount,fee,note,cb, getHash) {
                if (getHash == undefined)
                    getHash = true;
        ...
        var broadcaseHash = {
            ...
            "EXOTo": function (hash, obj, callback) {
                let fc = profileService.focusedClient;
                obj.signature = hash;
        ...
        eventBus.on('newMultiTrans', function(check){
            try {
                ...
                haveIn = true;
                var webHelper = require('Exoduscore/sendTransactionToNode');
                var chooseNetWork = "http://" + check.url + '/exchange';//self.chooseNetWork == undefined ? "http://relay.exo.one:9090/exchange" : self.chooseNetWork;
        ...
        this.submitData = function() {
            var objectHash = require('Exoduscore/object_hash.js');
            var fc = profileService.focusedClient;
            var value = {};
            var app;
        ...
        this.getPrivatePayloadSavePath = function(cb) {
            var fileName = 'ExodusPayment-' + $filter('date')(Date.now(), 'yyyy-MM-dd-HH-mm-ss') + '.' + configService.privateTextcoinExt;
            if (!isCordova) {
