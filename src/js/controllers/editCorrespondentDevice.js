...
angular.module('copayApp.controllers').controller('editCorrespondentDeviceController',
  function($scope, $rootScope, $timeout, configService, profileService, isCordova, go, correspondentListService, $modal, animationService,addressService) {
	...
	var self = this;
	var fc = $scope.index.walletType.EXO[0];
    ...
    self.address = '';
    self.stables = '';
    self.walletName = fc.walletName;
    self.image = fc.image;

    $timeout(function () {
        var wallet = require('Exoduscore/wallet.js');
        wallet.readAddressByWallet(fc.wallet,function (res) {
            self.address = res;
            ...
            for(let item in walletInfo){
                if(walletInfo[item].wallet == fc.wallet) self.stables = walletInfo[item].stables;
                break;
            }
            $scope.$apply();
        });
