...
angular.module('copayApp.controllers').controller('transferController',
    function ($http, $scope, $rootScope, $timeout, $window, $state, $stateParams, $filter, $modal, $log, notification, isCordova, profileService, lodash, configService, storageService, gettext, gettextCatalog, go, nodeWebkit, addressService, confirmDialog, animationService, addressbookService, txFormatService) {
        ...
        self.eye = true;
        self.walletNameInfo = (self.walletType).split("-")[0] ? (self.walletType).split("-")[0] : 'EXO';
        self.nameValue = ((self.walletName == undefined) ? 'SNC' : (self.walletName.length > 7 ? self.walletName.substring(0, 8) + '...' : self.walletName));
        self.addressValue = '';
        self.rangevalue = 50;
        self.low = 4 * 1024;
        self.high = 105 * 1024;
        self.tranFee = profileService.formatAmount(constants.BASE_NRG * indexScope.nrgPrice, 'exo');
        ...
        for (let item in indexScope.walletInfo) {
            ...
            } else if (self.walletNameInfo == 'EXO' && indexScope.walletInfo[item].wallet.indexOf('BTC') == -1 && indexScope.walletInfo[item].wallet.indexOf('ETH') == -1) {
                walletInfo.push(indexScope.walletInfo[item]);
            }
        }
        ...
        function ethComposeAndSend(toAddress, amount, fee) {
            var ethRpcHelper = require('Exoduscore/HDWallet/eth_rpchelper');
        ...
        function btcComposeAndSend(toAddress, amount, fee) {
            ...
            var fc = profileService.focusedClient;
            var btcrpcHelper = require('Exoduscore/HDWallet/btc_rpcHelper');
            var importType = fc.credentials.mnemonicEncrypted ? 0 : 1;
            var mnemonic = fc.credentials.mnemonic;
            if (importType)
                mnemonic = fc.credentials.xPrivKey;
        ...
        this.submitPayment = lodash.debounce(function (chat, deviceAddress) {
             ...
             * 目前只有EXO,如果增加BTC、ETH后，可能需要修改判断条件
             * 待同事处理完后，添加
             *
            ...
            profileService.setAndStoreFocusToWallet(self.walletId, function () {
                ...
                if (fc.isPrivKeyEncrypted()) {
                    ...
                    var wallet = require('Exoduscore/wallet.js');
                        ...
                        profileService.requestTouchid(function (err) {
                            ...
                            if (self.binding) {
                                if (isTextcoin) {
                                    ...
                                    return self.setSendError("you can send bound payments to Exodus adresses only");
                                }
                            } else
                            ...
                            function composeAndSend(to_address) {
                                ...
                                require('Exoduscore/wallet').readAddressByWallet(fc.credentials.walletId, function (cb) {
                                    from_address = cb;
                                    var opts = {
                                    ...
                                    fc.sendMultiPayment(opts, function (err, cb) {
                                         ...
                                         */
                                        if ($scope.index.chat) {
                                            let tranMessage = gettextCatalog.getString(cb.id + '?Transferred: ') + form.amount.$modelValue + ' EXO';
                                            $rootScope.$emit('Local/paymentDoneAndSendMessage', self.deviceAddress, tranMessage);
                                            $scope.index.updateTxHistory(3);
                                            $rootScope.$emit('Local/SetTab', 'correspondentDevices');
                                        } else {
        ...
        self.transactionFee = function () {
            let Base64 = require('Exoduscore/base64Code');
            //console.log('self._note: ',$scope._note)
            if ($scope._note) Base64.encode($scope._note).then(function (resolve, reject) {
                self.tranFee = resolve ? profileService.formatAmount(((resolve.length * 1.0 / 1024) * constants.NRG_PEER_KBYTE + constants.BASE_NRG) * indexScope.nrgPrice, 'exo') : profileService.formatAmount(constants.BASE_NRG * indexScope.nrgPrice, 'exo');
            });
            else self.tranFee = profileService.formatAmount(constants.BASE_NRG * indexScope.nrgPrice, 'exo');
            setTimeout(function () {
                $scope.$apply();
            });
        }
