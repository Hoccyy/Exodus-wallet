...
var eventBus = require('Exoduscore/event_bus.js');
var breadcrumbs = require('Exoduscore/breadcrumbs.js');
var device = require('Exoduscore/device.js');
...
angular.module('copayApp.controllers').controller('indexController', function ($rootScope, $scope, $log, $filter, $timeout, lodash, go, profileService, configService, isCordova, storageService, addressService, gettext, gettextCatalog, amMoment, nodeWebkit, txFormatService, uxLanguage, uxCurrency, $state, isMobile, addressbookService, notification, animationService, $modal, bwcService, backButton, promptBackupService, $anchorScroll, $location, newVersion) {
    ...
    var checkVerion = require('Exoduscore/checkVersion')

    var webHelper = require('Exoduscore/sendTransactionToNode');
    var bigNumber = require("bignumber.js");
    var jsonObject = JSON.parse('{}');
    var light = require('Exoduscore/light');
    self.relaySynTime = 0;
    var urlList = [];
    self.chooseNodeList = {};
    self.url = '';
    ...
    //             self.EXOTOBTC = data.exchangeRatios['EXO:BTC'].toString();
    //             // self.BTCTOEXO = data.exchangeRatios['BTC:EXO'].toString();
    //             self.BTCTOEXO = new bigNumber(1).dividedBy(data.exchangeRatios['BTC:EXO']).toString();
    //             self.ETHTOEXO = new bigNumber(1).dividedBy(data.exchangeRatios['ETH:EXO']).toString();
    //             self.EXOTOETH = data.exchangeRatios['EXO:ETH'].toString();
    //             //self.ETHTOEXO = data.exchangeRatios['ETH:EXO'].toString();
    //             self.EXOTOEXO = 1;
    //             self.ETHTOETH = 1;
    //             self.BTCTOBTC = 1;
    ...
    self.changesendType = function (type) {
        ...
        var type = self.showpopwallet ? self.showpopwallet : 'EXO';
        var length = self.walletInfo.length;
        for (var i = 0; i < length; i++) {
            var walletType = (walletInfo[i].wallet).split("-")[1] ? (walletInfo[i].wallet).split("-")[0] : 'EXO';
            if (walletType == type) {
    ...
    function updatePublicKeyRing(walletClient, onDone) {
        // var walletDefinedByKeys = require('Exoduscore/wallet_defined_by_keys.js');
    ...
    function sendBugReport(error_message, error_object) {
        var conf = require('Exoduscore/conf.js');
        var network = require('Exoduscore/network.js');
    ...
    eventBus.on('uncaught_error', function (error_message, error_object) {
        ...
        self.showErrorPopup(error_message, function () {
            var db = require('Exoduscore/db.js');
            db.close();
    ...
    self.openSubwalletModal = function () {
        ...
        var ModalInstanceCtrl = function ($scope, $modalInstance) {
            ...
            $scope.maExodusBalanceInfo = self.arrMaExodusBalances[self.assetIndex];
            $scope.asset = $scope.maExodusBalanceInfo.asset;
            ...
            for (var sa in assocSharedByAddress) {
                ...
                if (asset === 'base' || $scope.maExodusBalanceInfo.name)
            ...
            var walletDefinedByAddresses = require('Exoduscore/wallet_defined_by_addresses.js');
            async.eachSeries(
                arrSharedWallets,
        ...
        var ModalInstanceCtrl = function ($scope, $modalInstance) {
            ...
            */
            ...
            if (walletType !== '') {
                ...
                $scope.addressType = 'EXO';
                $scope.addressbookList('EXO');
            }
    ...
    self.resetSendForm = function () {
        ...
        setTimeout(function () {
            let tenexp = /^([a-zA-Z0-9]{10})(.*)([a-zA-Z0-9]{10})$/g;
            self.exTradeOutId = self.walletType.EXO[0].wallet;
            self.exExchangeFromId = self.walletType.EXO[0].walletId;
            self.exExchangeFromStable = self.walletType.EXO[0].stables;
            self.exExchangeFromName = self.walletType.EXO[0].walletName;
            self.exExchangeFromImg = 'exo';
            self.exExchangeFromAddr = self.walletType.EXO[0].address;
            self.exExchangeFromAddrShow = self.walletType.EXO[0].address.replace(tenexp, '$1...$3');
            self.moveRate = self[self.exExchangeFromImg.toUpperCase() + 'TO' + self.exExchangeToImg.toUpperCase()];
            /**
             * move
             * 默认设置兑换转入地址信息
    ...
    self.setBalance = function (assocBalances, assocSharedBalances) {
        ...
        if (!self.shared_address)
            self.arrMaExodusBalances = self.arrBalances;
        /*if (isCordova) {
    ...
    this.csvHistory = function () {
        ...
        $timeout(function () {
            fc.getTxHistory(self.arrBalances[self.assetIndex].asset, self.shared_address, function (txs) {
                ...
                var data = txs;
                var filename = 'Exodus-' + (self.alias || self.walletName) + '.csv';
                var csvContent = '';
    ...
    self.updateLocalTxHistory = function () {
        ...
        let ammount_SNC = 0;
        $timeout(function () {
            require('Exoduscore/wallet').getWalletsInfo(function (obj) {
                ...
                for (let item in trans) {
                    ...
                    } else if (trans[item].wallet.indexOf('EXO-') != -1) {
                        ammount_EXO = new bignumber(ammount_EXO).plus(trans[item].stablesNmuber).toString();
                        transEXO.push(trans[item]);
...
{ammount: number, ammountFro: *}, ammount_EXO: {ammount: number, ammountFro: *}}}
                 */
                self.ammount = {
                    ammount_BTC: {
                ...
                //self.ammount_EXO = profileService.formatAmount(ammount_EXO,'bytes');
                self.walletType.BTC = transBTC;
                self.walletType.ETH = transETH;
                self.walletType.EXO = transEXO;
                ...
                if (!count || !self.exExchangeFromImg) {
                     ...
                     * 默认设置兑换转出地址信息
                     */
                    self.exTradeOutId = transEXO[0].wallet;
                    self.exExchangeFromId = transEXO[0].walletId;
                    self.exExchangeFromStable = transEXO[0].stables;
                    self.exExchangeFromName = transEXO[0].walletName;
                    self.exExchangeFromImg = 'exo';
                    self.exExchangeFromAddr = transEXO[0].address;
                    self.exExchangeFromAddrShow = transEXO[0].address.replace(tenexp, '$1...$3');
                    self.moveRate = self[self.exExchangeFromImg.toUpperCase() + 'TO' + self.exExchangeToImg.toUpperCase()];
                ...
                let txTotal = [];
                for (let item in fc) {
                    let light = require('Exoduscore/light');
    ...
    self.updateHistoryFromNetwork = lodash.throttle(function () {
        setTimeout(function () {
            ...
            //todo delete
            // var lightWallet = require('Exoduscore/light_wallet.js');
    ...
    $rootScope.$on('Local/ShadowAddressForm', function (event, address) {
        var shadowWallet = require('Exoduscore/shadowWallet');
    ...
    var openTranInfoListener = $rootScope.$on('openTranInfo', function (event, tranId) {
        ...
        db.query('SELECT * FROM transactions WHERE id=?', [tranId], function (rows) {
            if (rows.length == 1) {
                ...
                $scope.btx.amountTl = rows[0].amount + rows[0].amount_point / configService.getUnitValue('exo');
                $scope.btx.amountStr = rows[0].amount + rows[0].amount_point / configService.getUnitValue('exo') + ' EXO';
                $scope.btx.feeStr = profileService.formatAmount(rows[0].fee * configService.getUnitValue('exo') + rows[0].fee_point, 'exo') + ' EXO';
    ...
    self.generatePubkey = function (address) {
        self.verificationAddress = "Exodus:" + address;
        self.showshadow = true;
        self.shadowstep = 'hot1';
        $timeout(function () {
            $rootScope.$apply();
     ...
     * 获取EXO市价
     */
    // self.updateExoPrice = function () {
    //     news.getExoData2(function (res) {
    //         if (!!res && res != null) {
    //             self.exodollar = res.page.list.EXO.price;//usd
    //             self.exormb = res.page.list.EXO.cnyPrice;//rmb
    //             $timeout(function () {
    //                 $scope.$apply();
    //             })
    //         }
     ...
     * @type {number}
     */
    // var updateExoPrice = setInterval(function () {
    //     self.updateExoPrice();
    // }, 5 * 1000);
    ...
    self.getAddress = function () {
        var webSocket = require('Exoduscore/webSocket.js');
        if (self.walletType) {//判断walletType是否赋值
            let address = self.walletType.EXO[0].address;
    ...
    self.resetTransactions = function () {
        ...
        async.series(arrQueries, function (err, res) {
            require('Exoduscore/light.js').updateStatu()
            $state.go('walletHome')
        });
    }
