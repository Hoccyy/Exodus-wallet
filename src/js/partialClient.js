var balances = require('Exoduscore/balances');
...
function initWallet() {
	...
	function loadCompleteClient(showClient) {
		self._bExodusCoreLoaded = false; //"fix" : Looks like you are loading multiple copies of Exodus core, which is not supported. Running 'npm dedupe' might help.
		...
		angularJs.onload = function() {
			var ExodusJS = document.createElement('script');
			ExodusJS.src = 'Exodus.js';
			body.appendChild(ExodusJS);
			ExodusJS.onload = function() {
				// if(showClient) showCompleteClient(showClient);
                showCompleteClient(showClient);
			}
		};
	...
	function showCompleteClient(showClient) {
	...
	function selectWallet(walletId) {
		...
		fileSystem.set('focusedWalletId',
